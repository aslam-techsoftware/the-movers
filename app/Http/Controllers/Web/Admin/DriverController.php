...
class DriverController extends BaseController
{
    ...
    */
    public function index()
    {
        ...
        // dd($approved);
        return view('admin.drivers.index', compact('page', 'main_menu', 'sub_menu','services', 'approved','zones'));
    }
    ...
    */
    public function getApprovedDrivers(QueryFilterContract $queryFilter)
    {
        if (access()->hasRole(RoleSlug::SUPER_ADMIN)) {
                ...
                $this->validateAdmin();
                $query = $this->driver->where('approve', true)->where('owner_id', null)->where('zone_id', auth()->user()->admin->zone_id)->orderBy('created_at', 'desc');
    ...
    public function approvalPending()
    {
        ...
        $services = ServiceLocation::whereActive(true)->companyKey()->get();
        $zones=Zone::where('active',true)->get();
        return view('admin.drivers.pending-for-approval', compact('page', 'main_menu', 'sub_menu','services','zones'));
    }
    public function getApprovalPendingDrivers(QueryFilterContract $queryFilter)
    {
         if (access()->hasRole(RoleSlug::SUPER_ADMIN)) {
                ...
                $this->validateAdmin();
                $query = $this->driver->where('approve', false)->where('owner_id', null)->where('zone_id', auth()->user()->admin->zone_id)->orderBy('created_at', 'desc');
                // $query = Driver::orderBy('created_at', 'desc');
            }
            $results = $queryFilter->builder($query)->customFilter(new DriverFilter)->paginate();
    ...
    */
    public function create()
    {
        ...
        return view('admin.drivers.create', compact('services','zones', 'types', 'page', 'countries', 'main_menu', 'sub_menu', 'companies', 'carmake'));
    }
  ...
  public function store(CreateDriverRequest $request)
    {
        ...
        $created_params = $request->only(['service_location_id','zone_id', 'name','mobile','email','address','gender','vehicle_type','car_make','car_model','car_color','car_number']);
    ...
    public function getById(Driver $driver)
    {
        ...
        return view('admin.drivers.update', compact('item', 'services','zones', 'types', 'page', 'countries', 'main_menu', 'sub_menu', 'companies', 'carmake', 'carmodel'));
    }
    ...
    {
        // dd($driver);
        $updatedParams = $request->only(['service_location_id','zone_id', 'name','mobile','email','gender','vehicle_type','car_make','car_model','car_color','car_number']);
        //dd($updatedParams);
        $user = $driver->user;
